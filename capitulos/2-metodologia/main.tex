...
O processo de modelagem do sistema partiu de alguns princípios elementares para a construção de um sistema de \textit{software}: 
...
\begin{itemize}
    ...
    % fáceis de serem alterados, de forma que as mudanças no sistema possam ser refletidas rapidamente nos modelos. Esse princípio é de extrema importância tanto para a extensibilidade do sistema quanto para o isolamento dos componentes em cenários de teste.
...
\begin{figure}[H] %\captionsetup{width=0.55\textwidth}%% Largura da legenda
...
Base de \textit{stacks} para cada sistema operacional
\end{minipage}.
...
Configuração do \textit{framework} SST e injeção das \textit{stacks} de infraestrutura
\end{minipage}.
}
...
Para publicar eventos no barramento, os serviços de \textit{backend} utilizaram o próprio \gls{sdk} da \gls{aws}.
...
\subsection{Aplicação de \gls{api}}
...
Implementação de todos os endpoits da \gls{api}, compatíveis para serem executados em funções \textit{Lambda}
\end{minipage}.
