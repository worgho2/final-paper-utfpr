...
\begin{itemize}
    \item \textbf{Usuário}: Módulo responsável por gerenciar as informações e permisssões dos usuários do sistema.
...
AWS Elastic Load Balancing \citep{awselasticloadbalancing} & sdk \textsuperscript{$\wedge$}3.592.0 & Balanceador de carga gerenciado utilizado para gerenciar as conexões entre o cliente web e o \textit{cluster} de \textit{containers} rodando o \textit{Gateway} de conexão. \\

\hline

AWS Elastic Container Service \citep{awsecsdocs} & sdk \textsuperscript{$\wedge$}3.592.0 & Serviço utilizado para a implantação do \textit{gateway} de conexão, no formato de cluster de \textit{containers} com escalabilidade previsível \\

\hline

AWS Elastic Container Registry \citep{awsecrdocs} & sdk \textsuperscript{$\wedge$}3.592.0 & Serviço utilizado para armazenar as imagens de \textit{container} do \textit{gateway} de conexão \\
