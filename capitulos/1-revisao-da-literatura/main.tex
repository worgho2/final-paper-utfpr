...
Existem diversos modelos de serviço disponíveis em ambientes de computação em nuvem, e eles são classificados de acordo com o nível de responsabilidade apresentado como requisito para consumir o serviço. \citep{awswhatiscloudcomputing}

O tipo de serviço que requer menor interferência nos aspectos de infraestrutura ou configuração é denominado \textit{Software as a Service} ou \textit{SaaS}. Nesse tipo de serviço, o usuário acessa diretamente uma aplicação, e o provedor da mesma é inteiramente responsável pelo gerenciamento e configuração da infraestrutura que o suporta.

Outro tipo de serviço é o \textit{Platform as a Service} ou \textit{PaaS}. Nele o usuário é capaz de executar e gerenciar aplicações sem a necessidade de se preocupar com a infraestrutura que suporta as aplicações.

O tipo de serviço que demanda o maior nível de responsabilidade do usuário é o \textit{Infrastructure as a Service} ou \textit{IaaS}. Nesse cenário, o usuário tem controle total sobre os recursos disponíveis.

A \textit{AWS} é um provedor de computação de nuvem que oferece a maior gama de serviços ao seus usuários e cada serviço possui objetivos e níveis de abstração particulares, podendo ser totalmente configuráveis ou camadas gerenciáveis de outros serviços.
...
Esse tipo de serviço é útil para soluções comuns como \textit{APIs} que necessitam na maioria dos casos dos mesmos componentes de infraestrutura como: Um servidor virtualizado de computação, configuração de rede, balanceador de carga, banco de dados e um local para armazenamento de objetos.
...
\section{Infraestrutura Virtual de Desktops (VDI)}
\label{sec:infraestruturaVirtualDeDesktops}
